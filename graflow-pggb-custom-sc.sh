...
### sc structural variants toy
fagz_files_sorted=('SGDref-0-genome.fa.gz' \
'TOY288-0-genome.fa.gz' \
...
docker run -it \
-v ${dir_dock}:/data \
-v ${HOME}/tools/pggb/pggb-${pggb_version}-custom/pggb-custom:/usr/local/bin/pggb:ro \
${USER}/pggb:${pggb_version} \
pggb -i /data/multi.fa.gz -s ${seg_length} \
-p ${pair_sim} -n ${n_aps} -t ${n_threads} \
-k ${match_filt} -P ${poa_param} -O ${poa_pad} \
-c ${wfmash_n} -l ${block_length} \
--mash-nmap-short-seq ${wfmash_n_short} \
-G ${poa_target_length} -o /data \
-V ${ref_strain_id}:${max_decomp}
...
pggb_graph=$(find "${dir_dock}" -maxdepth 1 -type f \
  -name "*${pggb_out_suffix}" -print -quit)

if [[ -z "${pggb_graph}" ]]; then
  echo "ERROR: PGGB final GFA not found in ${dir_dock}" >&2
  exit 1
fi

cp "${pggb_graph}" "${dir_gra}/${gfa_graph}"

### move VCF file to the variants folder
vcf_pref="${pggb_graph%.gfa}"
vcf_file="${vcf_pref}.${ref_strain_id}.decomposed.vcf"

if [[ ! -f "${vcf_file}" ]]; then
  echo "ERROR: decomposed VCF not found: ${vcf_file}" >&2
  exit 1
fi

cp "${vcf_file}" "${dir_calls}/bin-${ref_hyphen_hap}-deco.vcf"
